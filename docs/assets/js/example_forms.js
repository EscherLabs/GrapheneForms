  ...
  "basic": {
    ...
    "placeholder": {
        fields:[
            ...
            {label: 'Select', type: 'select', options:{options:[1,2,3], placeholder:'Choose One'}},
        ]
    },
    "columns": {
        fields:[
    ...
    "generate": {
        fields:[
        {label: 'Up to Ten, min defaults to 0',type:'select',options:{max:10}},
        {label: 'Start at two, step defaults to 1',type:'select',options:{min:2, max:10}},
        {label: 'Step By Threes',type:'select',options:{min:1, max:10,step:3}},
        {label: "Max must be set",type:'select',options:{ min:1, step:3}}
        ]
    },
    "format": {
        fields:[
        {label: 'Custom label', name: 'label',type:'select',options:{max:10, format:{ label:'Option {{index}}' }}},
        {label: 'Custom value', name: 'value',type:'select',options:{max:12,min:1, format:{ value:'option-{{value}}' }}},
        {label: 'Both', name: 'both',type:'select',options:{max:12,min:1, format:{ label:'{{value}} Column(s)' ,value:'col-md-{{value}}' }},help:'Select a bootstrap column class'}
        ]
    }
